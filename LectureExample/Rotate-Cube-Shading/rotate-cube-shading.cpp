...
int animationFlag =
    1;  // 1: animation; 0: non-animation. Toggled by key 'a' or 'A'
...
void display(void) {
    ...
    //              locations can be retrieved. This is needed to pass on values
    //              to uniform/attribute variables in shader ("variable binding"
    //              in shader).
    ...
    //    we need to use model-view matrix to transform light_position to Eye
    //    Frame.
    //    ==> Must be called *after* the model-view matrix mv for light position
    //    is
    //        set up.
    SetUp_Lighting_Uniform_Vars(mv);

    // The model-view matrix with all transformations for the cube
    mat4 model_view = mv * Scale(1.4, 1.4, 1.4) * RotateX(Theta[Xaxis]) *
...
void idle(void) {
    // Theta[Axis] += 0.01;
    Theta[Axis] +=
        1.0;  // YJC: change this value to adjust the cube rotation speed.
