 ...
 * Moodified by Yi-Jen Chiang to include the use of a general rotation function
   ...
   (The functions Rotate() and NormalMatrix() are added to the file
"mat-yjc-new.h" by Yi-Jen Chiang, where a new and correct transpose function
"transpose1()" and other related functions such as inverse(m) for the inverse of
3x3 matrix m are also added; see the file "mat-yjc-new.h".)
...
// 1: solid cube; 0: wireframe cube. Toggled by key 'c' or 'C'
int cubeFlag = 1;
...
point3 vertices[8] = {point3(-0.5, -0.5, 0.5),  point3(-0.5, 0.5, 0.5),
...
// quad(): generate two triangles for each face and assign colors to the
// vertices
...
void floor() {
    floor_colors[0] = vertex_colors[3];
    floor_points[0] = vertices[3];
    floor_colors[1] = vertex_colors[0];
    floor_points[1] = vertices[0];
    floor_colors[2] = vertex_colors[4];
    floor_points[2] = vertices[4];

    floor_colors[3] = vertex_colors[4];
    floor_points[3] = vertices[4];
    floor_colors[4] = vertex_colors[7];
    floor_points[4] = vertices[7];
    floor_colors[5] = vertex_colors[3];
    floor_points[5] = vertices[3];
}
//----------------------------------------------------------------------------
...
void init() {
    ...
    // Create and initialize a vertex buffer object for cube, to be used in
    // display()
    ...
    glBufferData(
        ...
        NULL, GL_STATIC_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(point3) * cube_NumVertices,
                    cube_points);
    glBufferSubData(GL_ARRAY_BUFFER, sizeof(point3) * cube_NumVertices,
                    sizeof(color3) * cube_NumVertices, cube_colors);
#endif
...
/*----- Set Up the Model-View matrix for the cube -----*/
#if 0 
    // The following is to verify the correctness of the function
    // NormalMatrix(): Commenting out Rotate() and un-commenting
    // mat4WithUpperLeftMat3() gives the same result.
    ...
    // * RotateY(angle);
    //
    // The set-up below gives the original scene (scene 1), using Correct
    // LookAt().
    //  mv = Translate(0.0, 0.5, 0.0) * mv * Scale (1.4, 1.4, 1.4)
    //               * Rotate(angle, 0.0, 2.0, 0.0);
    // * RotateY(angle);
    //
    // The set-up below gives the original scene (scene 1), when using
    // previously
...
#endif
#if 0  
    // The following is to verify that Rotate() about (3,0,0) is RotateX():
    ...
    // The set-up below gives the original scene (scene 1), using Correct
    // LookAt()
    //    mv = Translate(0.0, 0.0, 0.3) * LookAt(eye, at, up) * Scale
    //    (1.6, 1.5, 3.3);
    //
    // The set-up below gives the original scene (scene 1), when using
    // previously
    //       Incorrect LookAt() (= Translate(1.0, 1.0, 0.0) * correct LookAt() )
    //    mv = Translate(-1.0, -1.0, 0.3) * LookAt(eye, at, up) * Scale
    //    (1.6, 1.5, 3.3);
